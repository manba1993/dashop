...
import json
from tools.cache_dec import cache_check

#cache_page(30)修饰视图函数，可以在路由里修饰视图类
...
class GoodsIndexView(View):
    ...
    def get(self, request):
        ...
        for cata in all_catalog:
            cata_dict = {}
            cata_dict['catalog_id'] = cata.id  # 分类id
            cata_dict['catalog_name'] = cata.name  # 分类名
            spu_ids = SPU.objects.filter(catalog=cata).values('id')
            sku_list = SKU.objects.filter(spu__in=spu_ids, is_launched=True)[:3]  # 触发limit 查询
            cata_dict['sku'] = []
            for sku in sku_list:
                sku_dict = {}
                ...
                #sku.default_image_url  obj json不认识 ,需要转字符串
...
class GoodsDetailView(View):
    """
    获取sku 详情页信息，获取图片暂未完成
    """
  #找出物品也唯一标识做redis的，避免重复key_prefix：前缀，key_param：标识，cache：按需存储到缓冲的哪个缓存配置项
